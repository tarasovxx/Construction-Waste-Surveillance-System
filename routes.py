...
async def save_cropped_contours(video_path, contour_width_threshold=140, contour_height_threshold=140,
                                start_time='0:00', end_time='10:00', cropx=125, cropy=175,
    ...
    return output_video_path if os.path.isfile(output_video_path) else None, output_picture_path if os.path.isfile(
        output_picture_path) else None


@app.post("/process_video/{file_name}")
async def process_video(file_name: str, video: UploadFile):
    print(file_name)
    # Сохранить загруженное видео на сервер
    with open(file_name, "wb") as buffer:
        buffer.write(await video.read())

    # Вызвать функцию save_cropped_contours с помощью asyncio.run
    processed_video_path, preview_image_path = await asyncio.run(save_cropped_contours(file_name))
